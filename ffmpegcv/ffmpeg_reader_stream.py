...
from ffmpegcv.ffmpeg_reader import (
    get_videofilter_cpu, get_outnumpyshape, get_videofilter_gpu,
    get_num_NVIDIA_GPUs, decoder_to_nvidia)
...
class FFmpegReaderStreamNV(FFmpegReaderCAM):
    ...
    def VideoReader(
        ...
        (vid.crop_width, vid.crop_height), (vid.width, vid.height), (cropopt, scaleopt, filteropt) \
            = get_videofilter_gpu((vid.origin_width, vid.origin_height), pix_fmt, crop_xywh, resize, 
                resize_keepratio, resize_keepratioalign)
        vid.size = (vid.width, vid.height)
